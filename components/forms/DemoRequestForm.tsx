...
export default function DemoRequestForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.append("form_name", "demoRequest");
      formData.append("form_source", "Home Page - Request Demo Modal");
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      ...
      <div className="space-y-2">
        <label className="text-[13px] font-bold text-zinc-400 font-mono uppercase tracking-wider block">Comments</label>
        <textarea name="comments" rows={4} className="w-full bg-zinc-50 border border-zinc-200 rounded-lg p-4 text-[13px] text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-1 focus:ring-purple-600 focus:border-purple-600 hover:border-purple-900/30 transition-all resize-none font-normal" placeholder="How may we help you?" />
      </div>
