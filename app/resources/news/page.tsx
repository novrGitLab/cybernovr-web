  ...
  return (
    <div className="pt-24 md:pt-28 pb-24 px-4 sm:px-6 md:px-0 lg:px-margin-desktop max-w-[1536px] mx-auto space-y-12 bg-white text-zinc-900 antialiased">
      <div className="border-b border-zinc-200 pb-8 max-w-4xl text-left">
        ...
        <h1 className="text-3xl md:text-5xl font-extrabold text-zinc-900 tracking-tight uppercase mb-2">Latest News</h1>
          ...
          <article key={idx} className="bg-purple-950/[0.02] border border-purple-900/10 rounded-xl overflow-hidden flex flex-col justify-between shadow-sm group hover:border-purple-900/30 transition-all duration-300">
            <div className="space-y-0">
              ...
              <div className="p-6 space-y-3">
                <p className="text-sm font-mono font-bold text-red-700 uppercase tracking-widest">{news.source}</p>
                <h3 className="text-lg font-bold text-zinc-900 leading-snug group-hover:text-red-700 transition-colors uppercase tracking-tight">{news.title}</h3>
                <p className="text-zinc-600 text-xs md:text-base leading-relaxed font-normal">{news.desc}</p>
              </div>
            </div>
            <div className="px-6 pb-6">
              <a href={news.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-red-700 hover:text-red-600 font-bold text-sm uppercase tracking-wider transition-all font-mono group/btn">
      ...
      <section className="bg-purple-950/[0.02] border border-purple-900/10 rounded-2xl p-8 md:p-12 text-center space-y-4">
        ...
        <h2 className="text-xl md:text-3xl font-bold text-zinc-900 uppercase tracking-tight">Stay Informed</h2>
        <p className="text-base text-zinc-500 max-w-md mx-auto font-normal">
        ...
        <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto mt-4" onSubmit={(e) => { e.preventDefault(); alert("Thank you for subscribing!"); }}>
          ...
          />
          <button type="submit" className="bg-red-600 hover:bg-red-700 text-white font-black text-base uppercase tracking-widest px-6 py-3 rounded-lg transition-all shadow-md font-mono whitespace-nowrap">
            Subscribe
          </button>
        </form>
      </section>
