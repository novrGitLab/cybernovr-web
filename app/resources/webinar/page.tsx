  ...
  return (
    <div className="pt-24 md:pt-28 pb-24 px-4 sm:px-6 md:px-0 lg:px-margin-desktop max-w-[1536px] mx-auto space-y-12 bg-white text-zinc-900 antialiased">
      <div className="border-b border-zinc-200 pb-8 max-w-4xl text-left">
        ...
        <h1 className="text-3xl md:text-5xl font-extrabold text-zinc-900 tracking-tight uppercase mb-2">Webinars</h1>
        ...
        {webinars.map((web, idx) => (
          <div key={idx} className="p-6 bg-purple-950/[0.02] border border-purple-900/10 rounded-xl flex flex-col justify-between space-y-5 shadow-sm hover:border-purple-900/30 transition-all duration-300 group">
            <div className="space-y-3">
              <div className="flex items-center gap-1.5 text-sm font-mono font-bold text-zinc-400 uppercase tracking-widest">
                <Calendar className="h-3 w-3 text-red-700" /> {web.date}
              </div>
              <h3 className="text-lg font-bold text-zinc-900 uppercase tracking-tight leading-snug group-hover:text-red-700 transition-colors">{web.title}</h3>
              <p className="text-zinc-600 text-xs md:text-base leading-relaxed font-normal">{web.desc}</p>
            </div>
            <Link href="/contacts" className="inline-flex items-center justify-center gap-1.5 border-2 border-red-600 bg-white hover:bg-red-600 text-red-700 hover:text-white font-bold text-base uppercase tracking-widest px-5 py-3 rounded-lg transition-all font-mono shadow-sm group/btn">
